...
This method is implemented by creating three stacks: `Hard`, `Good`, and `Easy`. Initially, all falshcards are placed on the `Hard` stack. As the user revises, flashcards move between stacks based on how difficult they were for the user to recall. Those difficult to recall are placed on the `Hard` stack, those recalled successfully but might need more revision are placed on ther `Good` stack, and those that were very easy to recall are placed on the `Easy` stack. The revision algorithm prioritizes the `Hard` stack, followed by the `Good` stack. Revision continues untill all flashcards are on the `Easy` stack.
